...
// // Movement scale used by your integrator (kept from your version)
// static float ballSpeed = 75.0f;

// // Circle/collision radius
// static float ballRadius = 25.0f;

// // Boids tuning (in *world* units)
// static float perceptionRadius = 140.0f; // who I can "sense"
// static float separationRadius = 120.0f; // who I consider "too close"
// static float maxSpeed = 1.8f;           // target speed in *vel units* (x is moved by vel*ballSpeed)
// static float maxForce = 0.06f;          // steering strength (smaller = smoother)
...
//     {
//         // simple Euler (your scale retained: ballSpeed affects vel change and movement)
//         vel.x += acc.x * dt * ballSpeed;
//         vel.y += acc.y * dt * ballSpeed;

//         // keep velocity in check in *vel* units
//         vel = vlimit(vel, maxSpeed);

//         x += vel.x * dt * ballSpeed;
//         y += vel.y * dt * ballSpeed;

//         // reset acceleration for next frame (we'll add forces before the next integrate)
//         acc = Vector2{0, 0};
...
//     // --- Flocking: Separation + Alignment + Cohesion (robust & fast) ----------
...
//         int countPAC = 0; // for alignment+cohesion
//         int countSEP = 0; // for separation only
...
//             {
//                 // Alignment & cohesion pool
//                 sumVel = Vector2Add(sumVel, b->vel);
//                 sumPos = Vector2Add(sumPos, Vector2{b->x, b->y});
//                 ++countPAC;

//                 // Separation (only if very close)
//                 if (d2 <= sr2)
//                 {
//                     const float d = sqrtf(d2);
//                     // Push away stronger when very close (1/d falloff)
...
// // Elastic resolution for equal-mass discs
...
//         return; // no overlap / zero distance
...
//     {
//         // perfectly elastic (e = 1), equal masses (m = 1)
//         const float j = -(1.0f + 1.0f) * velAlongNormal / 2.0f; // (-(1+e)*v) / (1/m1+1/m2) = -2*v / 2
//         const float ix = j * nx;
//         const float iy = j * ny;
...
//     // positional correction to resolve overlap (prevents jitter)
//     const float penetration = (2.0f * ballRadius) - dist;
//     if (penetration > 0.0f)
//     {
//         const float percent = 0.8f; // correction percent
...
//         // update bounding boxes after positional change
...
//     (float)sizeX, (float)sizeY, 8, 0 // slightly larger capacity before subdivision
// );
...
//         balls.emplace_back(b);
//         // tree will be rebuilt below before we query
//     }

//     // --- 0) Make sure the tree reflects current positions before querying
//     qt->rebuild(balls);

//     const float dt = GetFrameTime();

//     // --- 1) Flocking forces (use perception radius query)
...
//         // IMPORTANT: pass by const ref (no copy), and correct formulas inside flock()
//         a->flock(neighbors);
//     }
...
//     // --- 3) Rebuild for post-integration positions (authoritative)
//     qt->rebuild(balls);

//     // --- 4) Broadphase (tight) + narrowphase collisions
...
//     // Toggle to visualize tree (keep off for perf)
